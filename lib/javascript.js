    ...
    if (err) {
      ...
      files.forEach(function(file) {
        ...
        var result = UglifyJS.minify(
          ...
          {
            sourceMap: {
              filename : file.split(skelly.appRoot)[1].split('.js')[0] + '.min.' + skelly.hash + '.js',
              url:file.split(skelly.appRoot)[1].split('.js')[0] + '.min.' + skelly.hash + '.js.map'
            }
          }
        );
        // need to remove the physical path to the sourcemap
        result.map = result.map ? result.map.replace(skelly.appRoot,'') : /* istanbul ignore next */ ' ';
